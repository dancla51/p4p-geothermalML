...
def TikhonovNEWSolveRMSE(filename, pTrain, pTest, k=5, TikLambda1=0, TikLambda2=0, suppress=False):
    ...
    for j in range(T):
        # fill in m values
        for i in range(n):
            y[T*i+j, 0] = mReal[i,j] * np.sqrt(float(Lambda1)) * dTrain[i,j]  # Seems to have no effect on result but will affect RMSE score
        # fill in M values
        y[T*n+j, 0] = MReal[0,j]
    ...
    for i in range(n):
        for j in range(T):
            # fill in d values
            A1[T*i+j, T*i+j] = np.sqrt(float(Lambda1)) * dTrain[i,j]
            # fill in z values
